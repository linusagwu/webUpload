...
if(checkUserSession($db) !== True){
	if(!empty($_POST["username"]) && !empty($_POST["password"]) && !empty($_POST["re_password"])){
		...
		$profilePicture = "https://i.stack.imgur.com/l60Hf.png    "; //default
						...
						if(strpos($username, " ") === False){
							if($password === $re_password){
								...
								if(mysqli_num_rows($sql) < 1){
									...
									mysqli_query($db, "INSERT INTO user(id, firstName, lastName, profilePicture, username, password, session, admin,verified,joinned_time, email, user_type) VALUES (NULL, '$firstName', '$lastName ', '$profilePicture', '$username', '$password', '$userSession', 0, 0, '$dateTime', '$email', '$user_type')");
									...
									$registerStatus = array("success" => false, "message" => "Username already exist!");
								}
